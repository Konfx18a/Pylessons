...
async def create_user(db: Annotated[Session, Depends(get_db)], created_user: CreateUser):
    test_user = db.scalar(select(User).where(User.username == created_user.username))
    if not test_user:
